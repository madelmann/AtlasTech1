...
bool CLoadMD2::ImportMD2(t3DModel *pModel, char *strFileName, char *strTexture)
{
	...
	if(!m_FilePointer) 
	{
		...
		sprintf_s(strMessage, "ERROR[ImportMD2]: '%s' not found!\n", strFileName);
		Console->Output(strMessage);
		return false;
	}
	...
	if(m_Header.version != 8)
	{
		...
		sprintf_s(strMessage, "ERROR[ImportMD2]: Invalid file format (Version not 8): %s!\n", strFileName);
		Console->Output(strMessage);
		return false;
	}
...
void CLoadMD2::ConvertDataStructures(t3DModel *pModel)
{
	...
	for (i = 0; i < pModel->numOfObjects; i+= 1)
	{
		...
		SAFE_DELETE_ARRAY ( m_pFrames[i].pVertices );
...
void CLoadMD2::CleanUp()
{
	...
	SAFE_DELETE_ARRAY ( m_pSkins );				// Free the skins data
	SAFE_DELETE_ARRAY ( m_pTexCoords );			// Free the texture coord data
	SAFE_DELETE_ARRAY ( m_pTriangles );			// Free the triangle face data
	SAFE_DELETE_ARRAY ( m_pFrames );			// Free the frames of animation
}
