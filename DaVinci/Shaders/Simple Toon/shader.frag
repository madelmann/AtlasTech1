...
vec3 eyePos = vec3(0.0,0.0,5.0);										// Eye position
vec3 lightPos = vec3(0.0,5.0,5.0);										// Light postion
...
void main(void)
{
	...
	vec3 EyeLight = normalize(LightVert + EyeVert);						// Simple Silhouette
	...
	{
		gl_FragColor = color0;											// Specular part
		...
		if (spec < 0.2)
			...
			gl_FragColor = color2;										// Diffuse part
